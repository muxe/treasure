<template>
  <div id="app">
    ...
    <div>
      <span>Total Payments: {{readableTotalPayments}}</span>
    </div>
    <div>
      <span>My Payments: {{readableMyPayments}}</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async claimTreasure () {
      ...
      console.log('claimTreasure', passwordSha);
      let result = await treasureContract.claimTreasure(passwordSha);
      console.log('claimTreasure', result);
    },
    async getTreasure () {
      let password = this.inputPassword;
      let passwordSha = this.createHash(password);
      console.log('addTreasure passwordSha', passwordSha);
      passwordSha = '0x' + passwordSha;
      // passwordSha = Web3.utils.asciiToHex(passwordSha)
      passwordSha = Web3.utils.toHex(passwordSha)
      console.log('getTreasure', passwordSha, Web3.utils.isHex(passwordSha));
      let result = await treasureContract.getTreasureByHash(passwordSha);
      console.log('getTreasure', result);
    },
    createHash (input) {
